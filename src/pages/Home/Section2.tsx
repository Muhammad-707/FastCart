      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-8">
        <div className="flex flex-col gap-5 w-full md:w-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-6 sm:gap-12 md:gap-16">
            <h2 className="text-3xl md:text-4xl font-bold tracking-tight text-zinc-900 dark:text-white">Flash Sales</h2>
            <div className="flex items-center gap-2 sm:gap-3">
              <Timer label="Days" value={timeLeft.days} />
              <span className="text-xl text-[#DB4444] font-black animate-pulse">:</span>
              <Timer label="Hours" value={timeLeft.hours} />
              <span className="text-xl text-[#DB4444] font-black animate-pulse">:</span>
              <Timer label="Minutes" value={timeLeft.minutes} />
              <span className="text-xl text-[#DB4444] font-black animate-pulse">:</span>
              <Timer label="Seconds" value={timeLeft.seconds} />
            </div>
          </div>
        </div>

        <div className="flex gap-3 self-end md:self-auto">
          <button className="flash-prev p-3.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/80 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 active:scale-95 dark:text-white shadow-sm transition-all duration-200">
            <ArrowLeft size={18} />
          </button>
          <button className="flash-next p-3.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/80 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 active:scale-95 dark:text-white shadow-sm transition-all duration-200">
            <ArrowRight size={18} />
          </button>
        </div>
      </div>
