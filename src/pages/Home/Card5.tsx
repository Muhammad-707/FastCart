...
export default function Card5({ product }: { product: any }) {
  ...
  const toggleWishlist = (e: React.MouseEvent) => {
    ...
    if (itemIndex > -1) {
      ...
      updatedWishlist.push({ id: productId, _id: productId, image: imageUrl, title: productName, price, rating, reviews: quantity, isNew, colors });
    }
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.stopPropagation();
    dispatch(addToCart({ id: String(productId), name: productName, image: imageUrl, price: Number(price) || 0, quantity: 1 }));
  ...
  return (
    ...
    >
      <div className="relative w-full h-[250px] bg-zinc-100 dark:bg-zinc-900 rounded-sm flex items-center justify-center overflow-hidden">
        ...
        <img src={imageUrl} alt={productName} className="h-[150px] object-contain transition-transform duration-300 group-hover:scale-105" />
